...
CREATE TABLE `default`.`Eleve`  (
  ...
  `langue` varchar(30) NOT NULL DEFAULT 'INDEFINI',
...
CREATE TABLE `default`.`Salle`  (
  ...
  `position` varchar(100) NOT NULL DEFAULT  'INDEFINI',
...
ALTER TABLE `default`.`Residents` ADD CONSTRAINT `habite` FOREIGN KEY (`idPers`) REFERENCES `default`.`Personne` (`idPersonne`) ON DELETE NO ACTION ON UPDATE CASCADE;
